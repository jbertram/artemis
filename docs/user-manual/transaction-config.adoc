...
To cope with this {project-name-full} can, if configured, scan for old transactions and rollback any it finds.
...
Please note that {project-name-full} will not unilaterally rollback any XA transactions in a prepared state - this must be heuristically rolled back via the management API if you are sure they will never be resolved by the transaction manager.
