...
To stop the {project-name-full} instance you will use the same `artemis` script, but with the `stop` argument.
Example:
...
Please note that {project-name-full} requires a Java 11 or later.
...
The configuration for the {project-name-full} core broker is contained in `broker.xml`.

There are many attributes which you can configure for {project-name-full}.
