...
By default, {project-name-full} consumers buffer messages from the server in a client side buffer before the client consumes them.
This improves performance: otherwise every time the client consumes a message, {project-name-full} would have to go the server to request the next message.
...
To prevent this, {project-name-full} pre-fetches messages into a buffer on each consumer.
...
{project-name-full} also can limit the amount of data sent from a client to a server to prevent the server being overwhelmed.

=== Window based flow control

In a similar way to consumer window based flow control, {project-name-full} producers, by default, can only send messages to an address as long as they have sufficient credits to do so.
