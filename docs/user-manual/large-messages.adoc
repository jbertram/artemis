...
{project-name-full} can be configured to give special treatment to messages which are beyond a configured size.
...
[NOTE]
====
{project-name-full} messages are encoded using 2 bytes per character so if the message data is filled with ASCII characters (which are 1 byte) the size of the resulting {project-name-full} message would roughly double.
...
{project-name-full} supports setting the body of messages using input and output streams (`java.lang.io`)
...
When using JMS, {project-name-full} maps the streaming methods on the core API (see ClientMessage API table above) by setting object properties . You can use the method `Message.setObjectProperty` to set the input and output streams.
...
If you choose not to use the `InputStream` or `OutputStream` capability of {project-name-full} You could still access the data directly in an alternative fashion.
