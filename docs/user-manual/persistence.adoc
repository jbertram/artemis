...
As delete records are added to the journal, {project-name-full} has a sophisticated file garbage collection algorithm which can determine if a particular journal file is needed any more - i.e. has all its data been deleted in the same or other files.
If so, the file can be reclaimed and re-used.

{project-name-full} also has a compaction algorithm which removes dead space from the journal and compresses up the data so it takes up less files on disk.
...
{project-name-full} ships with two implementations:
...
With AIO, {project-name-full} will be called back when the data has made it to disk, allowing us to avoid explicit syncs altogether and simply send back confirmation of completion when AIO informs us that the data has been persisted.
...
The standard {project-name-full} core server uses two instances of the journal:
...
+
By default {project-name-full} will try and use an AIO journal.
...
For large messages, {project-name-full} persists them outside the message journal.
This is discussed in xref:large-messages.adoc#large-messages[Large Messages].

{project-name-full} can also be configured to page messages to disk in low memory situations.
This is discussed in xref:paging.adoc#paging[Paging].

If no persistence is required at all, {project-name-full} can also be configured not to persist any data at all to storage as discussed in the Configuring the broker for Zero Persistence section.
...
If you choose `ASYNCIO` but are not running Linux or you do not have libaio installed then {project-name-full} will detect this and automatically fall back to using `NIO`.
Choosing `MAPPED` chooses the Java Memory Mapped journal.

journal-sync-transactional::
If this is set to true then {project-name-full} will make sure all transaction data is flushed to disk on transaction boundaries (commit, prepare and rollback).
The default value is `true`.

journal-sync-non-transactional::
If this is set to true then {project-name-full} will make sure non transactional message data (sends and acknowledgements) are flushed to disk each time.
...
When {project-name-full} starts and there is no initial message data, {project-name-full} will pre-create `journal-min-files` number of files.
+
...
The Java NIO journal gives great performance, but If you are running {project-name-full} using Linux Kernel 2.6 or later, we highly recommend you use the `ASYNCIO` journal for the very best persistence performance.
...
Configuring {project-name-full} to perform zero persistence is straightforward.
