...
The function of a bridge is to consume messages from a source queue, and forward them to a target address, typically on a different {project-name-full} server.
...
In summary, bridges are a way to reliably connect two separate {project-name-full} servers together.
With a core bridge both source and target servers must be {project-name-full} servers.
...
Core bridges are for linking an {project-name-full} node with another {project-name-full} node and do not use the JMS API.
