...
{project-name-full} also ships with a wide range of examples, many of which demonstrate JMS API usage.
...
We'll also show how to configure and use JNDI, and also how to use JMS with {project-name-full} without using any JNDI.
...
In other words, no objects are stored in JNDI on the {project-name-full} server, instead they are simply instantiated on the client based on the provided configuration.
...
The following configuration properties _are strictly required when {project-name-full} is running in stand-alone mode_.
When {project-name-full} is integrated to an application server (e.g. Wildfly) the application server itself will almost certainly provide a JNDI client with its own properties.
====
...
Here's a simple example of the JNDI context environment for a client looking up a connection factory to access an _embedded_ instance of {project-name-full}:
...
The property _value_ should be the name of the queue hosted by the {project-name-full} server.
...
{project-name-full} supports the direct instantiation of JMS Queue, Topic and ConnectionFactory instances, so you don't have to use JNDI at all.
