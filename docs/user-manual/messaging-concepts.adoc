...
{project-name-full} provides client implementations which are a fully compliant with xref:using-jms.adoc#using-jms-or-jakarta-messaging[JMS 1.1 & 2.0 as well as Jakarta Messaging 2.0 & 3.0].
...
{project-name-full} provides its own core client API for clients to use if they wish to have access to functionality over and above that accessible via the JMS API.

Please see xref:core.adoc#core-api[Core] for using the Core API with {project-name-full}.
...
{project-name-full} provides very configurable state-of-the-art clustering model where messages can be intelligently load balanced between the servers in the cluster, according to the number of consumers on each node, and whether they are ready for messages.

{project-name-full} also has the ability to automatically redistribute messages between nodes of a cluster to prevent starvation on any particular node.
...
{project-name-full} bridges can be configured with filter expressions to only forward certain messages, and transformation can also be hooked in.

{project-name-full} also allows routing between queues to be configured in server side configuration.
