...
{project-name-full} has an extensive _management API_ that allows a user to modify a server configuration, create new resources (e.g. addresses and queues), inspect these resources (e.g. how many messages are currently held in a queue) and interact with it (e.g. to remove messages from a queue).
{project-name-full} also allows clients to subscribe to management notifications.

There are numerous ways to access {project-name-full} management API:
...
* Using the Core Client -- management operations are sent to {project-name-full} server using _Core Client messages_
...
Although there are four different ways to manage {project-name-full}, each API supports the same functionality.
...
+
The `ActiveMQServerControl` exposes {project-name-full} server configuration through all its attributes (e.g. `getVersion()` method to retrieve the server's version, etc.)
...
{project-name-full} allows to start and stop its remote resources (acceptors, diverts, bridges, etc.) so that a server can be taken off line for a given period of time without stopping it completely (e.g. if other management operations must be performed such as resolving heuristic transactions).
These resources are:
...
{project-name-full} can be managed using http://www.oracle.com/technetwork/java/javase/tech/javamanagement-140525.html[JMX].

The management API is exposed by {project-name-full} using MBeans.
By  default, {project-name-full} registers its resources with the domain  `org.apache.activemq.artemis`.
...
Managing {project-name-full} using JMX is identical to management of any Java Applications using JMX.
It can be done by reflection or by creating proxies of the MBeans.

=== Configuring JMX

By default, JMX is enabled to manage {project-name-full}.
...
Due to the authorisation which is enabled by default {project-name-full}  can _not_ be managed locally using JConsole when connecting as a _local  process_.
...
By default, {project-name-full} server uses the JMX domain "org.apache.activemq.artemis".
To manage several {project-name-full} servers from the _same_ MBeanServer, the JMX domain can be configured for each individual {project-name-full} server by setting `jmx-domain` in `broker.xml`:
...
See the xref:examples.adoc#jmx-management[JMX Management Example] which shows how to use a remote connection to JMX and MBean proxies to manage {project-name-full}.
...
<!-- {project-name-full} using management messages    -->
...
See the xref:examples.adoc#management[Management Example] which shows how to use JMS messages to manage the {project-name-full} server.

== Management Notifications

{project-name-full} emits _notifications_ to inform listeners of potentially interesting events (creation of new resources, security violation, etc.).
...
{project-name-full} defines a special _management notification address_.
...
{project-name-full}'s Core JMS Client can be used to receive notifications:
...
Message counters can be used to obtain information on queues _over time_ as {project-name-full} keeps a history on queue metrics.
...
[,java]
----
// retrieve a connection to {project-name-full}'s MBeanServer
