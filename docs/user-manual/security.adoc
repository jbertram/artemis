...
{project-name-full} contains a flexible role-based security model for applying security to queues, based on their addresses.

As explained in xref:core.adoc#using-core[Using Core], {project-name-full} core consists mainly of sets of queues bound to addresses.
...
{project-name-full} allows sets of permissions to be defined against the queues based on their address.
...
{project-name-full} ships with a user manager that reads user credentials from a file on disk, and can also plug into JAAS or JBoss Application Server security.
...
When messaging clients are connected to servers, or servers are connected to other servers (e.g. via bridges) over an untrusted network then {project-name-full} allows that traffic to be encrypted using the Secure Sockets Layer (SSL) transport.
...
As mentioned above, the xref:command-line-interface[command-line interface] supports a command to `add` a user.
Commands to `list` (one or all) users, `remove` a user, and `reset` a user's password and/or role(s) are also supported via the xref:command-line-interface[command-line interface] as well as the normal management interfaces (e.g. JMX, web console, etc.).
...
{project-name-full} supports SCRAM-SHA-256 and SCRAM-SHA-512 SASL mechanisms to provide authentication for AMQP connections.
...
On the server, a Kerberos or SCRAM-SHA JAAS authenticated Principal must be added to the Subject's principal set as an {project-name-full} UserPrincipal using the corresponding {project-name-full} `Krb5LoginModule` or `SCRAMLoginModule` login modules.
They are separate to allow conversion and role mapping to be as restrictive or permissive as desired.

The <<propertiesloginmodule,PropertiesLoginModule>> or <<ldaploginmodule,LDAPLoginModule>> can then be used to map the authenticated  Principal to an {project-name-full} <<role-based-security-for-addresses,Role>>.
Note that in the case of Kerberos, the Peer Principal does not exist as an {project-name-full} user, only as a role member.

In the following example, any existing Kerberos authenticated peer will convert to an {project-name-full} user principal and will have role mapping applied by the LDAPLoginModule as appropriate.
...
These functions are also available from the xref:command-line-interface[command-line interface].
