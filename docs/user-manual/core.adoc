...
{project-name-full} core is a messaging system with its own API.
...
* {project-name-full} also supports the sending/consuming of very large messages much larger than can fit in available RAM at any one time.
...
This unique {project-name-full} feature, allows you to have full guarantees that sent messages have reached the server without having to block on each message sent until a response is received.
