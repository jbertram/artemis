...
Filter expressions are used in several places in {project-name-full}
...
There are some differences between JMS selector expressions and {project-name-full} core filter expressions.
Whereas JMS selector expressions operate on a JMS message, {project-name-full} core filter expressions operate on a core message.
...
Since {project-name-full} supports STOMP clients which can only send messages with string properties, that restriction is a bit limiting.
...
{project-name-full} also supports special https://en.wikipedia.org/wiki/XPath[XPath] filters which operate on the _body_ of a message.
