...
{project-name-full}'s addresses can be assigned an expiry address so that, when messages are expired, they are removed from the queue and sent to the expiry address.
...
Using the {project-name-full} Core API you can set an expiration time directly on the message:
