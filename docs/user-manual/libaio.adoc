...
{project-name-full} distributes a native library, used as a bridge for its fast journal, between {project-name-full} and Linux libaio.
...
These are the native libraries distributed by {project-name-full}:
...
When using libaio, {project-name-full} will always try loading these files as long as they are on the xref:using-server.adoc#library-path[library path]
