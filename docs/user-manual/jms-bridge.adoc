...
{project-name-full} includes a fully functional JMS message bridge.
...
The JMS Bridge may be useful for fast transfers as this chapter covers, but keep in mind that more complex scenarios requiring transformations will require you to use a more advanced transformation system that will play on use cases that will go beyond {project-name-full}.
====
...
A bridge can be deployed as a standalone application or as a web application managed by the embedded Jetty instance bootstrapped with {project-name-full}.
The source and the target can be located in the same virtual machine or another one.

The bridge can also be used to bridge messages from other non {project-name-full} JMS servers, as long as they are JMS 1.1 compliant.
...
A xref:core-bridges.adoc#core-bridges[core bridge]) is used to bridge any two {project-name-full} instances and uses the core API.
...
{project-name-full} doesn't ship with such an implementation, but if you are running within an Application Server you can inject the Transaction Manager that is shipped.
...
The configuration example above uses the default implementation provided by {project-name-full} that looks up the connection factory using JNDI.
...
In the configuration example above, we have used the default provided by {project-name-full} that looks up the destination using JNDI.
...
If both the source and the destination are on the same {project-name-full} server instance then this can be achieved by sending and acknowledging the messages in the same local transaction.
...
Please see xref:examples.adoc#jms-bridge[JMS Bridge Example] which shows how to programmatically instantiate and configure a JMS Bridge to send messages to the source destination and consume them from the target destination between two standalone {project-name-full} brokers.
