...
{project-name-full} supports _interceptors_ to intercept packets entering and exiting the server.
...
The interceptors can also be run on the {project-name-full} client side to intercept packets either sent by the client to the server or by the server to the client.
