...
{project-name-full} clusters allow groups of {project-name-full} servers to be grouped together in order to share message processing load.
Each active node in the cluster is an active {project-name-full} server which manages its own messages and handles its own connections.
...
We'll also discuss client side load balancing, where we can balance client connections across the nodes of the cluster, and we'll consider message redistribution where {project-name-full} will redistribute messages between nodes to avoid starvation.
...
However, care must be taken when copying the {project-name-full} files.
Do not copy the {project-name-full} _data_ (i.e. the `bindings`, `journal`, `paging`, and `large-messages` directories) from one node to another.
...
This information, let's call it the Cluster Topology, is actually sent around normal {project-name-full} connections to clients and to other servers over cluster connections.
...
There can be many broadcast groups per {project-name-full} server.
...
Make sure the file is in the java resource path so that {project-name-full} can load it.
...
If you want {project-name-full} to use this stacks for channel creation, you have to make sure the value of `jgroups-file` in your broadcast-group/discovery-group configuration to be the name of this jgroups configuration file.
...
Discovery groups are used in two places in {project-name-full}:
...
Although a discovery group will always accept broadcasts, its current list of available primary and backup servers is only ever used when an initial connection is made, from then server discovery is done over the normal {project-name-full} connections.
...
There can be many discovery groups defined by {project-name-full} server.
...
Let's discuss how to configure an {project-name-full} client to use discovery to discover a list of servers to which it can connect.
...
If cluster connections are defined between nodes of a cluster, then {project-name-full} will load balance messages arriving at a particular node from a client.
...
There can be zero or more cluster connections defined per {project-name-full} server.
...
By having multiple cluster connections on different addresses a single {project-name-full} Server can effectively take part in multiple clusters simultaneously.
+
...
Note that {project-name-full} will _not_ forward messages to other nodes if there are no _queues_ of the same name on the other nodes, even if this parameter is set to `STRICT`.
Using `STRICT` is like setting the legacy `forward-when-no-consumers` parameter to `true`.
+
If this is set to `ON_DEMAND` then {project-name-full} will only forward messages to other nodes of the cluster if the address to which they are being forwarded has queues which have consumers, and if those consumers have message filters (selectors) at least one of those selectors must match the message.
...
{project-name-full} can be configured to also load balance messages to nodes which might be connected to it only indirectly with other {project-name-full} servers as intermediates in a chain.
+
This allows {project-name-full} to be configured in more complex topologies and still provide message load balancing.
We'll discuss this more later in this chapter.
+
The default value for this parameter is `1`, which means messages are only load balanced to other {project-name-full} serves which are directly connected to this server.
This parameter is optional.
...
When creating connections between nodes of a cluster to form a cluster connection, {project-name-full} uses a cluster user and cluster password which is defined in `broker.xml`:
...
If they are not changed from the default, {project-name-full} will detect this and pester you with a warning on every start-up.
====

== Client-Side Load balancing

With {project-name-full} client-side load balancing, subsequent sessions created using a single session factory can be connected to different nodes of the cluster.
...
{project-name-full} provides four out-of-the-box load balancing policies, and you can also implement your own and use that.
...
With message redistribution {project-name-full} can be configured to automatically _redistribute_ messages from queues which have no consumers or consumers with filters that don't match messages.
...
The attribute `match` can be an exact match or it can be a string that conforms to the {project-name-full} wildcard syntax (described in xref:wildcard-syntax.adoc#wildcard-syntax[Wildcard Syntax]).
...
{project-name-full} clusters can be connected together in many different topologies, let's consider the two most common ones here
...
{project-name-full} supports scaling down a cluster with no message loss (even for non-durable messages).
...
{project-name-full} can be configured to do just that.
