...
Support for transactional acknowledgements is not implemented in {project-name-full}.
The `ACK` frame can not be part of a transaction.
It will be ignored if its `transaction` header is set.

=== Virtual Hosting

{project-name-full} currently doesn't support virtual hosting, which means the `host` header in `CONNECT` frame will be ignored.
...
In {project-name-full}, these destinations are mapped to _addresses_ and _queues_ depending on the operation being done and the desired semantics (e.g. anycast or multicast).
...
{project-name-full} also supports STOMP over https://html.spec.whatwg.org/multipage/web-sockets.html[Web Sockets].
...
With this configuration, {project-name-full} will accept STOMP connections over Web Sockets on the port `61614`.
...
The `stomp-websockets` xref:examples.adoc[example] shows how to configure an {project-name-full} broker to have web browsers and Java applications exchanges messages.
