...
If a `redelivery-delay` is specified, {project-name-full} will wait this delay before redelivering the messages.
...
{project-name-full}'s addresses can be assigned a dead letter address.
...
In normal use, {project-name-full} does not update delivery count _persistently_ until a message is rolled back (i.e. the delivery count is not updated _before_ the message is delivered to the consumer).
...
As this behavior breaks strict JMS semantics, {project-name-full} allows to persist delivery count before message delivery but this feature is disabled by default due to performance implications.
