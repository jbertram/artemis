...
By default all passwords in {project-name-full} server's configuration files are in plain text form.
...
{project-name-full} can be configured to use 'masked' passwords in its configuration files.
...
When {project-name-full} loads a masked password it uses the codec to decode it back into the real password.

{project-name-full} provides a default codec.
...
{project-name-full}' default JAAS security manager uses plain properties files where the user passwords are specified in a hashed form by default.
...
If using the legacy `mask-password` and `password-codec` values then when a `connector` or `acceptor` is initialised, {project-name-full} will add these values to the parameters using the keys `activemq.usemaskedpassword` and `activemq.passwordcodec` respectively.
...
----
+
This indicates the cluster password is a masked value and {project-name-full} will use <<the-default-codec,the default codec>> to decode it.
...
For user's convenience {project-name-full} provides a default codec.
...
Simply make sure the codec is in {project-name-full}'s classpath.
...
When {project-name-full} reads the cluster-password it will initialize the `NewCodec` and use it to decode "mask_password".
