...
In this chapter we'll describe the concepts required for understanding {project-name-full} transports and where and how they're configured.

== Acceptors

One of the most important concepts in {project-name-full} transports is the _acceptor_.
...
Each acceptor defines a way in which connections can be made to the {project-name-full} server.
...
Out of the box, {project-name-full} currently uses https://netty.io/[Netty], a high performance low level network library.
...
{project-name-full} supports using a single port for all protocols, {project-name-full} will automatically detect which protocol is being used CORE, AMQP, STOMP, MQTT or OPENWIRE and use the appropriate {project-name-full} handler.
...
batchDelay::
Before writing packets to the transport, {project-name-full} can be configured to batch up writes for a maximum of `batchDelay` milliseconds.
...
remotingThreads::
{project-name-full} will, by default, use a number of threads equal to three times the number of cores (or hyper-threads) as reported by `Runtime.getRuntime().availableProcessors()` for processing incoming packets.
...
This allows {project-name-full} to use native sockets/io instead of Java NIO.
...
{project-name-full} will by default enable the corresponding native transport if a supported platform is detected.

If running on an unsupported platform or any issues loading native libs, {project-name-full} will fallback onto Java NIO.
...
With single port support {project-name-full} will now automatically detect if http is being used and configure itself.
