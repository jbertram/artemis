...
In this section we will discuss connection time-to-live (TTL) and explain how {project-name-full} deals with crashed clients and clients which have exited without cleanly closing their resources.

== Cleaning up Resources on the Server

Before an {project-name-full} client application exits it is considered good practice that it should close its resources in a controlled manner, using a `finally` block.
...
{project-name-full} supports client reconnection, so we don't want to clean up "dead" server side resources too soon or this will prevent any client from reconnecting, as it won't be able to find its old sessions on the server.

{project-name-full} makes all of this configurable via a _connection TTL_.
...
If you fail to do so, {project-name-full} will detect this at garbage collection time, and log a warning (If you are using JMS the warning will involve a JMS connection).

{project-name-full} will then close the connection / client session for you.
